...
@section('content')
    <x-adminlte-card>
        @can('crear-periodos')
            <a class="btn btn-primary" href="{{ route('periodos.create') }}" role="button"><i class="fa fa-plus"></i> Nuevo Periodo</a>
        @endcan
        ...
        <div class="card-body">
            ...
            @endphp
            <x-adminlte-datatable id="table1" :heads="['Id', 'Mes', 'Año', 'Nombre', 'Estado', 'Acciones']" head-theme="dark" :config=$config striped hoverable
                with-buttons>
                @foreach ($periodos as $periodo)
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-info" href="{{ route('periodos.show', $periodo->id) }}"
                                role="button"><i class="far fa-eye"></i></a>
                            @can('editar-periodos')
                                <a class="btn btn-success" href="{{ route('periodos.edit', $periodo->id) }}"
                                    role="button"><i class="fas fa-pencil-alt"></i></a>
                            @endcan
                            <a class="btn btn-danger" href="{{ route('asociados.pdf', $periodo->id) }}" role="button"><i
