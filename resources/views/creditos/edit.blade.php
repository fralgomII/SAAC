...
@section('content_header')
    <h1 class="m-0 text-dark">Editar Solicitud de Crédito</h1>
@stop

@section('content')
    <x-adminlte-card>
        <form method="POST" action="{{ route('creditos.update', $credito->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                @php
                ...
                <x-adminlte-input-date id="fecha_credito" name="fecha_credito" label="Fecha de la Solicitud" :config="$config"
                    placeholder="Seleccione fecha" fgroup-class="col-md-2" :value="$credito->fecha_credito">
                ...
                <x-adminlte-select name="asociado_id" label="Nombre del asociado" fgroup-class="col-md-5">
                    ...
                    @foreach ($asociados as $asociado)
                        <option value="{{ $asociado->id }}" {{ $credito->asociado_id == $asociado->id ? 'selected' : '' }}>
                            {{ $asociado->nombre }} {{ $asociado->primer_apellido }} {{ $asociado->segundo_apellido }}
                        </option>
                    @endforeach
                </x-adminlte-select>
                <x-adminlte-select name="lineacredito_id" label="Línea de Crédito" fgroup-class="col-md-4">
                    ...
                    @foreach ($lineacreditos as $lineacredito)
                        <option value="{{ $lineacredito->id }}" data-interes="{{ $lineacredito->interes }}"
                            ...
                            data-seguro-vida="{{ $lineacredito->seguro_vida }}"
                            data-seguro-credito="{{ $lineacredito->seguro_credito }}"
                            {{ $credito->lineacredito_id == $lineacredito->id ? 'selected' : '' }}>
                            {{ $lineacredito->nombre }}
                        </option>
                    @endforeach
                </x-adminlte-select>
            ...
            <div class="row">
                <x-adminlte-input id="valor_credito_input" name="valor_credito" type="text" label="Valor Crédito"
                    ...
                    onchange="formatoNumero()" value="{{ $credito->valor_credito }}" />
                <x-adminlte-input name="interes_anual" id="interes_anual" type="number" label="Interés Anual(%)"
                    placeholder="1" fgroup-class="col-md-2" class="text-right" min="8" max="100" step="0.01"
                    value="{{ $credito->interes_anual }}" />
                <x-adminlte-input name="interes_credito" id="interes_credito" type="number" label="Interés (%)"
                    placeholder="1" fgroup-class="col-md-2" class="text-right" min="0.5" max="100" step="0.01"
                    value="{{ $credito->interes_credito }}" />
                ...
                <x-adminlte-input name="plazo_credito" id="plazo_credito" type="number" label="Plazo (en meses)"
                    ...
                    onchange="calcularCuota()" value="{{ $credito->plazo_credito }}" />
            </div>
            ...
            <div class="row">
                ...
                <x-adminlte-select name="estado" label="Estado del Crédito" fgroup-class="col-md-2">
                    ...
                    <option value="Solicitado" {{ $credito->estado == 'Solicitado' ? 'selected' : '' }}>Solicitado
                    </option>
                    <option value="En Estudio" {{ $credito->estado == 'En Estudio' ? 'selected' : '' }}>En Estudio
                    </option>
